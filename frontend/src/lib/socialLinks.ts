import { GFGLogo } from "@/components/logos/GFGLogo";
import { LeetcodeLogo } from "@/components/logos/LeetcodeLogo";
import { Github, Linkedin, Twitter } from "lucide-react";

export const socialLinks = [
  { icon: Github, href: "https://github.com/flabingo", label: "Github" },
  { icon: Linkedin, href: "https://linkedin.com/in/satyam-prajapati-m", label: "LinkedIn" },
  { icon: Twitter, href: "https://x.com/flabingo16", label: "Twitter" },
  { icon: LeetcodeLogo, href: "https://leetcode.com/flabingo", label: "Leetcode" },
  { icon: GFGLogo, href: "https://www.geeksforgeeks.org/profile/flabingo", label: "GeeksforGeeks"}
];